...
import { LogedInUser } from "../model.js";
...
export async function expressAuthentication(
    ...
    if (securityName !== "jwt") {
        return Promise.reject({ status: 401, message: "Unknown security schema" });
    }
    ...
    if (!token) {
        return Promise.reject({ status: 401, message: "Missing token" });
    }
